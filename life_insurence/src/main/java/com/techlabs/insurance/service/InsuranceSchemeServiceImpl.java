...
public class InsuranceSchemeServiceImpl implements InsuranceSchemeService {
    ...
    public Message addScheme(AddSchemeDto addSchemeDto) {
        ...
        if (addSchemeDto.getSchemeImage() != null) {
            byte[] imageBytes = Base64.getDecoder().decode(addSchemeDto.getSchemeImage());
            insuranceScheme.setSchemeImage(imageBytes);
        }
    ...
    public List<GetSchemeDto> getScheme(long planId) {
        ...
        for (InsuranceScheme sc : planOpt.get().getScheme()) {
        	   if (sc.getInsurancePlan() == null) {
                  
                   System.err.println("Scheme ID " + sc.getSchemeId() + " has no associated InsurancePlan.");
                   continue;
               }
	...
	@Override
	public Message addScheme(MultipartFile schemeImage) {
